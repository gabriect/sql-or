...
-- retornar os telefones de uma pessoa
SELECT 
    p.nome, --nomes dos atributos
    t.numero_telefone --nomes dos atributos
FROM 
    tb_pessoa p,
    TABLE(p.telefone) t --quebra o varray
WHERE 
    p.nome = 'Fernanda Souza';
...
SELECT 
    ...
    COUNT(a.cod_barras) AS total_alimentos_consumidos
FROM 
    tb_pessoa p, 
    TABLE(p.telefone) t,  -- Acessando o VARRAY de telefones
    TABLE(p.lista_alimentos) a  -- Acessando a NESTED TABLE de alimentos
GROUP BY 
    p.nome, 
    t.numero_telefone;


SELECT CP.data_hora_compra,
    DEREF(CP.pessoa).nome AS Cliente,
    DEREF(CP.pessoa).endereco.cep AS Cep_Cliente
FROM tb_compra_pessoa CP;