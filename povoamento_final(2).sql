...
-- teste metodo descrição usuario
DECLARE
    v_pessoa tp_pessoa;  -- variável para armazenar a pessoa
    v_descricao VARCHAR2(4000);  -- variável para armazenar a descrição
BEGIN
    -- Selecionando a pessoa inserida
    SELECT VALUE(p) INTO v_pessoa FROM tb_pessoa p WHERE p.id_usuario = 1;

    -- Chamando o método descricao_usuario
    v_descricao := v_pessoa.descricao_usuario;
...
-- teste função alimentos consumidos
DECLARE
    v_empresa tp_empresa;  -- variável para armazenar a empresa
    v_total NUMBER;  -- variável para armazenar o total de alimentos
BEGIN
    -- Selecionando a empresa recém-inserida
    SELECT VALUE (e) INTO v_empresa FROM tb_empresa e WHERE e.id_usuario = 101;

    -- Chamando a função contar_alimentos_consumidos
    v_total := v_empresa.contar_alimentos_consumidos;

    -- Exibindo o resultado
    DBMS_OUTPUT.PUT_LINE('Total de alimentos consumidos por ' || v_empresa.nome || ': ' || v_total);
END;
/

-- teste metodo descrição usuario
DECLARE
    v_empresa tp_empresa;  -- variável para armazenar a pessoa
    v_descricao VARCHAR2(4000);  -- variável para armazenar a descrição
BEGIN
    -- Selecionando a pessoa inserida
    SELECT VALUE(e) INTO v_empresa FROM tb_empresa e WHERE e.id_usuario = 101;
    ...
    -- Chamando o método descricao_usuario
    v_descricao := v_empresa.descricao_usuario;
...
-- teste metodo order function
DECLARE
    v_evento tp_evento;
    TYPE tp_lista_eventos IS TABLE OF tp_evento; -- lista para guardar os eventos
    v_lista_eventos tp_lista_eventos := tp_lista_eventos(); -- inicializa a lista vazia
BEGIN
    -- Lê eventos da tabela (assumindo que já existem)
    SELECT VALUE (e)
    BULK COLLECT INTO v_lista_eventos FROM tb_evento e;

    -- Ordenando os eventos em ordem decrescente de qtd_pessoas
    FOR i IN 1 .. v_lista_eventos.COUNT - 1 LOOP
        FOR j IN i + 1 .. v_lista_eventos.COUNT LOOP
            IF v_lista_eventos(i).compare_by_qtd_pessoas(v_lista_eventos(j)) < 0 THEN
                -- Troca de eventos
                v_evento := v_lista_eventos(i);
                v_lista_eventos(i) := v_lista_eventos(j);
                v_lista_eventos(j) := v_evento;
            END IF;
        END LOOP;
    END LOOP;

    -- Exibindo os eventos ordenados
    FOR i IN 1 .. v_lista_eventos.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE('Evento ID: ' || v_lista_eventos(i).id_evento || 
                             ', Qtd Pessoas: ' || v_lista_eventos(i).qtd_pessoas);
    END LOOP;
END;
/
...
-- teste da map function
DECLARE
    v_plano tp_plano_de_assinatura;  -- variável para armazenar o plano
    v_total DECIMAL(10, 2);  -- variável para armazenar o valor total
BEGIN
    -- Selecionando o plano básico
    SELECT VALUE (p) INTO v_plano FROM tb_plano_de_assinatura p WHERE p.id_plano = 1;

    -- Chamando a MAP MEMBER FUNCTION
    v_total := v_plano.calcular_valor_total;

    -- Exibindo o resultado
    DBMS_OUTPUT.PUT_LINE('Valor total do plano ' || v_plano.nivel || ': ' || v_total);
END;
/
-- testar a member procedure
DECLARE
    v_plano tp_plano_de_assinatura;  -- Variável para armazenar o plano
    preco_anterior DECIMAL(10, 2);    -- Variável para armazenar o preço anterior
BEGIN
    -- Seleciona o plano com id_plano = 1
    SELECT VALUE (p) INTO v_plano FROM tb_plano_de_assinatura p WHERE p.id_plano = 1;

    -- Armazena o preço atual em uma variável
    preco_anterior := v_plano.preco;

    -- Atualiza o preço do plano
    v_plano.atualizar_preco(150.00);  -- Define o novo preço

    -- Exibe o preço anterior e o novo preço
    DBMS_OUTPUT.PUT_LINE('Preço anterior do plano ' || v_plano.nivel || ': ' || preco_anterior);
    DBMS_OUTPUT.PUT_LINE('Novo preço do plano ' || v_plano.nivel || ': ' || v_plano.preco);
END;
/
